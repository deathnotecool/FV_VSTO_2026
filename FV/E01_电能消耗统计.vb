...
Public Class E01_电能消耗统计

    '声明作用域为类级的对象,该对象建立了与数据库的连接,此时数据库为Access.
    Dim objConnection As New OleDbConnection _
       ("Provider=Microsoft.Ace.OleDb.12.0;Data Source=D:\2 笔记记录\0 过程信息管理笔记\能资源消耗管理\能资源消耗管理.accdb")  '家里台式机
    '("Provider=Microsoft.Ace.OleDb.12.0;Data Source=D:\2_公司专用\3笔记记录\0_过程信息管理笔记\设备管理\设备管理.accdb")  '三星笔记本
    '("Provider=Microsoft.Ace.OleDb.12.0;Data Source=F:\2 笔记记录\8 过程信息管理\能资源消耗管理\能资源消耗管理.accdb")  '家里台式机
    ...
    Dim objDataAdapter As New OleDbDataAdapter("SELECT 电能消耗.* FROM 电能消耗 ORDER BY 编号", objConnection)
    ...
    Private Sub FillDataSetAndView()
        ...
        '向DataSet对象填充由SqlDataAdapter对象SelectCommand属性从数据库检索到的数据.. 
        '注意:Fill方法使用选择命令SelectCommand.Connection.如果该链接已打开,就会自动打开填充数据后保持打开连接对象,反之则反.  
        objDataAdapter.Fill(objDataSet, "dn")  '表(dn)是初始构建起来的,命名为dn.
        objDataView = New DataView(objDataSet.Tables("dn"))   '初始化并构建DataView对象.
        'CurrencyManager(窗体获取到的数据记录集合)对象集合包含于BindingContect(内置于Win窗体,无须创建)中,将DataView对象转化为CurrencyManager对象。
    ...
    Private Sub BindFields()
        ...
        myArray = {"编号", "月份", "主要耗能设备", "目标运行时长", "实际运行时长"， "电量消耗", "电费费用", "备注说明"}
        ...
        For i = 0 To UBound(myArray)
            ...
            If GroupBox1.Controls(myArray(i).ToString).Name = "电费费用" Then GroupBox1.Controls(myArray(i).ToString).Text _
            = Format(CType(GroupBox1.Controls(myArray(i).ToString).Text, Integer), "￥#,##0.00") '转换货币格式类型.
        Next i
    ...
    Private Sub ShowPosition()
        GroupBox1.Controls("电费费用").Text _
            = Format(CType(GroupBox1.Controls("电费费用").Text, Integer), "￥#,##0.00")
...
" Of " & objCurrencyManager.Count()                                 '显示当前记录位置,并标记记录数. 
    ...
    '加载窗体触发事件
    Private Sub E01_电能消耗统计_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        grdAuthorTitles.DataMember = "dn"           '设置gird控件要显示的数据源(具体的表名称).
        Dim objAlignRightCellStyle As New DataGridViewCellStyle                       '初始化DataGridViewCellStyle对象(作为grd控件单元格或标题样式实例) 
        objAlignRightCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft   '将对齐方式格式改为垂直居中向左对齐,从而能对运行状态字段进行对齐.
        ...
        grdAuthorTitles.Columns(0).HeaderText = "编号"   '设置控件列标题   
        grdAuthorTitles.Columns(1).HeaderText = "月份"
        grdAuthorTitles.Columns(2).HeaderText = "主要耗能设备"
        grdAuthorTitles.Columns(3).HeaderText = "目标运行时长"
        grdAuthorTitles.Columns(4).HeaderText = "实际运行时长"
        grdAuthorTitles.Columns(5).HeaderText = "电量消耗"
        grdAuthorTitles.Columns(6).HeaderText = "电费费用"
        grdAuthorTitles.Columns(7).HeaderText = "备注说明"

        grdAuthorTitles.Columns(7).Width = 390 '设置指定列默认宽度大一点
        ''改变字段标题名称和样式'Change column names and styles using the column name  


        grdAuthorTitles.Columns("备注说明").HeaderCell.Value = "备注"              '重新设置列标题的值显示为"描述"
        grdAuthorTitles.Columns("备注说明").HeaderCell.Style = objAlignRightCellStyle  '标题重新调用列标题样式(之前设定的-居中右对齐)
        grdAuthorTitles.Columns("电费费用").DefaultCellStyle = objCurrencyCellStyle    '单元格内容重新调用样式(之前设定的-垂直右对齐)
    ...
    Private Sub 执行排序_Click(sender As Object, e As EventArgs) Handles 执行排序.Click
        ...
        Select Case 排序字段.SelectedIndex      'Determine the appropriate item selected and set the Sort property of the DataView object..            
            ...
            Case 0
                objDataView.Sort = "编号"   '按字段设备编号升序排序,下同.
            Case 1
                objDataView.Sort = "月份"
            Case 2
                objDataView.Sort = "主要耗能设备"
            Case 3
                objDataView.Sort = "目标运行时长"
            Case 4
                objDataView.Sort = "实际运行时长"
            Case 5
                objDataView.Sort = "电量消耗"
            Case 6
                objDataView.Sort = "电费费用"
            Case 7
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If 查询条件.Text.Length = 0 Then              '如果是空值
            E01_电能消耗统计_Load(Nothing, Nothing)   '调用加载窗体事件.填充数据显示DateGirdVie完整视图,绑定控件,显示当前记录位置
        End If
    End Sub
   ...
   ("SELECT TOP 1 * FROM 电能消耗 ORDER BY 序列号 DESC", objConnection)  '存贮当前记录位置给变量  Save the current record position..
        objConnection.Open()   '打开数据库连接 Open the connection, execute the command SELECT TOP 1 * FROM 表名 ORDER BY 排序字段 DESC
        ...
        Select Case Len(intMaxID.ToString)
            Case 1
                strID = "DN00" & intMaxID.ToString
            Case 2
                strID = "DN0" & intMaxID.ToString
            Case Else
                strID = "DN" & intMaxID.ToString
        End Select
        objCommand.Connection = objConnection '设置命令对象的属性 Set the SqlCommand object properties..'将连接字符串的连接对象赋值给Connection属性
        '维修单号.Enabled = True'将CommandText属性(要执行的SQL字符串)设置指定的值
        'myArray = {"序列号", "姓名", "性别", "出生年月", "技术职称", "专业等级", "发证日期", "有效期至", "证件编号"}

        '{"编号", "月份", "主要耗能设备", "目标运行时长", "实际运行时长"， "电量消耗", "电费费用", "备注说明"}
        objCommand.CommandText = "INSERT INTO 电能消耗 " &
    "(编号, 月份, 主要耗能设备, 目标运行时长, 实际运行时长, 电量消耗, 电费费用，备注说明) " &
    "VALUES(@编号, @月份, @主要耗能设备, @目标运行时长, @实际运行时长, @电量消耗, @电费费用，@备注说明)"
        ...
        objCommand.Parameters.AddWithValue("@编号", strID)          '指定参数写入值,下同.
        objCommand.Parameters.AddWithValue("@月份", 月份.Text)
        objCommand.Parameters.AddWithValue("@主要耗能设备", 主要耗能设备.Text)
        objCommand.Parameters.AddWithValue("@目标运行时长", 目标运行时长.Text) '转换日期类型
        objCommand.Parameters.AddWithValue("@实际运行时长", 实际运行时长.Text)
        objCommand.Parameters.AddWithValue("@电量消耗", 电量消耗.Text)
        objCommand.Parameters.AddWithValue("@电费费用", 电费费用.Text).DbType = DbType.Currency
        ...
        E01_电能消耗统计_Load(Nothing, Nothing)         '调用方法填充数据到指定字段及绑定控件  Fill the dataset and bind the fields..
    ...
    Private Sub 更新_Click(sender As Object, e As EventArgs) Handles 更新.Click
        ...
        objCommand.Connection = objConnection '设置命令对象一些属性 Set the SqlCommand object properties..
        'SQL语句表示按照指定条件,更新表设备名称  "放置地点", "制造商", "制造日期", "使用部门", "运行状态"等
        '  myArray = {"编号", "月份", "主要耗能设备", "目标运行时长", "实际运行时长"， "电量消耗", "电费费用", "备注说明"}
        objCommand.CommandText = "UPDATE 电能消耗统计 " &
        "SET 月份 = @月份,主要耗能设备 = @主要耗能设备,目标运行时长 = @目标运行时长,实际运行时长 = @实际运行时长,电量消耗 = @电量消耗,电费费用 = @电费费用,
备注说明 = @备注说明 WHERE 编号 = @编号"  ' '接着使用SQL字符串设置CommandText属性.
        ...
        objCommand.Parameters.AddWithValue("@月份", 月份.Text)
        objCommand.Parameters.AddWithValue("@主要耗能设备", 主要耗能设备.Text)
        objCommand.Parameters.AddWithValue("@目标运行时长", 目标运行时长.Text)
        objCommand.Parameters.AddWithValue("@实际运行时长", 实际运行时长.Text)
        objCommand.Parameters.AddWithValue("@电量消耗", 电量消耗.Text)
        objCommand.Parameters.AddWithValue("@电费费用", 电费费用.Text).DbType = DbType.Date  '转换类型.
        objCommand.Parameters.AddWithValue("@备注说明", 备注说明.Text).DbType = DbType.Date  '转换类型..
        objCommand.Parameters.AddWithValue _
        ("@编号", BindingContext(objDataView).Current("编号"))
        ...
        objConnection.Close()                '关闭数据库连接  Close the connection..
        E01_电能消耗统计_Load(Nothing, Nothing) '调用方法显示数据和绑定字段  Fill the DataSet and bind the fields..
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        objCommand.CommandText = "DELETE FROM 电能消耗统计 " &
        "WHERE 编号 = @编号"
    ...
    ("@编号", BindingContext(objDataView).Current("编号"))
        ...
        E01_电能消耗统计_Load(Nothing, Nothing)
    ...
    Private Sub 退出_Click(sender As Object, e As EventArgs) Handles 退出.Click
        ...
        Globals.Ribbons.Ribbon1.btn电能.Enabled = True
        Me.Close()
    End Sub

    '关闭
    Private Sub E01_电能消耗统计_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        '清理内存及数据适配器对象
        objDataAdapter = Nothing           '清理数据适配器对象,释放内存 ' Clean up
        objConnection = Nothing         '清理连接对象,释放内存
        Globals.Ribbons.Ribbon1.btn电能.Enabled = True
