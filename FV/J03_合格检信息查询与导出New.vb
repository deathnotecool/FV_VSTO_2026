...
Public Class J03_合格检信息查询与导出New

    Dim objConnection2th As New SqlConnection("server=192.168.3.52,1433\SQLEXPRESS;database=合格检验系统确认NEW;User ID=sa;Password=85773627;")  '张一帆服务器

    'Dim objConnection2th As New sqlConnection _
    ...
    '("Provider=Microsoft.Ace.sql.12.0;Data Source=F:\2 笔记记录\8 过程信息管理\进销存管理\进销存管理.accdb")  '家里台式机
    ' ("Provider=Microsoft.Ace.sql.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\不良品信息管理.accdb")  '公司共享盘
    ...
    'Dim objDataAdapter As New sqlDataAdapter("SELECT 保养.* FROM 保养 ORDER BY 保养单号", objConnection2th)
    ...
    Private Sub J03_合格检信息查询与导出New_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        '为信息种类复合框设置项目
        With 信息种类       '引用复合框对象
            .Items.Add（"合格检信息"）     '添加项目为合同基本信息
        End With                                '结束引用语句
    ...
    Private Sub 信息种类_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 信息种类.SelectedIndexChanged
        ...
        '初始化sqlCommand类的一个实例,并将其分配给SelectCommand属性.   Set the SelectCommand properties..
    ...
    Private Sub 开始查询_Click(sender As Object, e As EventArgs) Handles 开始查询.Click
        ...
        ElseIf 运算符.Text = "like" Then       '否则如果复合框的值等于"like",那么执行
            ...
            Condition = Condition & " like '%" & 条件值1.Text & "%'"       '重新给变量赋值SQL语句
    ...
    Private Sub 数据导出_Click(sender As Object, e As EventArgs) Handles 数据导出.Click
        ...
        With ws                     '引用工作表
            For i = 0 To a   '在0到字段数量上-1处循环
                ...
                If .Cells(1, i + 1).value = "备注说明" Then .Cells(1, i + 1).value = "IDTag & 备注说明"
        ...
        xlapp.Columns("L:L").ColumnWidth = 15
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select * from 合格检信息"
        objDataAdapter1th.Fill(objDataSet1th, "wxxx4")
        'objDataView1th = New DataView(objDataSet1th.Tables("wxxx3"))
    ...
    Private Sub 关闭窗体_Click(sender As Object, e As EventArgs) Handles 关闭窗体.Click
        ...
        Globals.Ribbons.Ribbon1.btnSerachConformityInformathingNew.Enabled = True
        myArray = Nothing
        Me.Close()
    End Sub

    Private Sub F05_索赔信息查询与导出_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        关闭窗体_Click(Nothing, Nothing)
    End Sub
