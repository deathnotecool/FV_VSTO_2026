...
' myArray = {"ID", "报告单号", "报告日期", "客户", "供应商", "材质", "品名", "检验项目", "目的区分", "报告来源", "检测依据", "合格确认", "文件资料",  "备注"}
Public Class I01_检测试验信息管理

    '声明作用域为类级的对象,该对象建立了与数据库的连接,此时数据库为Access.
    Dim objConnection1th As New OleDbConnection _
      ("Provider=Microsoft.Ace.OleDb.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\文件管理.accdb")  '公司共享盘
    ...
    Dim objDataAdapter As New OleDbDataAdapter("SELECT 检测试验信息.* FROM 检测试验信息 ORDER BY ID", objConnection1th)
    ...
    Private Sub BindFields()
        ...
        '第三参数为要绑定给控件的数据字段(列表).
        For i = 0 To UBound(myArray)
            If GroupBox1.Controls(myArray(i).ToString).Name = "合格确认" Then
    ...
    Private Sub ShowPosition()
        ...
        Catch e As System.Exception   '声明一个错误变量类型
            ...
            GroupBox1.Controls("报告日期").Text = CType(Now, String)
    ...
    '加载窗体触发事件
    Private Sub I01_检测试验信息_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        grdAuthorTitles.Columns(1).HeaderText = "报告单号"
        grdAuthorTitles.Columns(2).HeaderText = "报告日期"
        grdAuthorTitles.Columns(3).HeaderText = "客户"
        grdAuthorTitles.Columns(4).HeaderText = "供应商"
        grdAuthorTitles.Columns(5).HeaderText = "材质"
        grdAuthorTitles.Columns(6).HeaderText = "品名"
        grdAuthorTitles.Columns(7).HeaderText = "检验项目"
        grdAuthorTitles.Columns(8).HeaderText = "目的区分"
        grdAuthorTitles.Columns(9).HeaderText = "报告来源"
        grdAuthorTitles.Columns(10).HeaderText = "检测依据"
        grdAuthorTitles.Columns(11).HeaderText = "合格确认"
        grdAuthorTitles.Columns(12).HeaderText = "文件资料"
        grdAuthorTitles.Columns(13).HeaderText = "备注"
        grdAuthorTitles.Columns(13).Width = 50 '设置指定列默认宽度大一点

        '改变字段标题名称和样式'Change column names and styles using the column name  
        grdAuthorTitles.Columns("品名").HeaderCell.Value = "产品名称" '重新设置列标题的值显示为"描述"
        '标题重新调用列标题样式(之前设定的-居中右对齐)
        grdAuthorTitles.Columns("品名").HeaderCell.Style = objAlignRightCellStyle
        '单元格内容重新调用样式(之前设定的-货币样式)

        '改变字段标题名称和样式'Change column names and styles using the column name  
        grdAuthorTitles.Columns("合格确认").HeaderCell.Value = "合格打✔" '重新设置列标题的值显示为"描述"
        ...
        For i As Integer = 0 To grdAuthorTitles.RowCount - 2                           '有一个空白行也算一行
            'If i = grdAuthorTitles.RowCount - 1 Then Exit For '运行最后一行空白值会显示Nothing,会出错
            If CType(grdAuthorTitles.Item(11, i).Value.ToString(), Boolean) Then
                grdAuthorTitles.Rows(i).DefaultCellStyle.Font = New Font("宋体", 9, FontStyle.Regular)    '构建一个字体类及相关属性
                grdAuthorTitles.Rows(i).DefaultCellStyle.ForeColor = Color.Black    '字体颜色设置为黑色
        ...
        目的区分.Items.Clear()             '给组合框添加项目  'Add items to the combo box..
        目的区分.Items.Add("供应商开发") ： 目的区分.Items.Add("常规检查") ： 目的区分.Items.Add("产品设计开发") ： 目的区分.Items.Add("客户要求") : 目的区分.Items.Add("其他")
        ...
        客户.Items.Add("苏州斗山") ： 客户.Items.Add("VOLVO") ： 客户.Items.Add("-")
        材质.Items.Clear()
        材质.Items.Add("S48C-Q/T") ： 材质.Items.Add("S48C-Nor.") ： 材质.Items.Add("S48C-Nor.+Q/T") ： 材质.Items.Add("42CrMo4-Q/T") ： 材质.Items.Add("42CrMo-Q/T") ： 材质.Items.Add("-")
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "项目" & " from " & "标准规范要求 ORDER BY 项目" '写入SQL语句
        ...
        检验项目.Items.Clear()                               '清楚复合框项目集
        For inCounter = 0 To tb.Rows.Count - 1               '在表行数上循环
            检验项目.Items.Add(tb.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
        Next

        objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "供应商" & " from " & "供应商及品名 ORDER BY 供应商" '写入SQL语句
        objDataSet1th = New DataSet()                        '数据适配器对象开始检索数据并填充到DataSet对象
    ...
    Private Sub 执行排序_Click(sender As Object, e As EventArgs) Handles 执行排序.Click
        ...
        Select Case 排序字段.SelectedIndex      'Determine the appropriate item selected and set the Sort property of the DataView object..            
            ...
            Case 6
                objDataView.Sort = "品名"
            Case 7
                objDataView.Sort = "检验项目"
            Case 8
                objDataView.Sort = "目的区分"
            Case 9
                objDataView.Sort = "报告来源"
            Case 10
                objDataView.Sort = "检测依据"
            Case 11
                objDataView.Sort = "合格确认"
            Case 12
                objDataView.Sort = "文件资料"
            Case 13
                objDataView.Sort = "备注"
        End Select
    ...
    Private Sub 执行查询_Click(sender As Object, e As EventArgs) Handles 执行查询.Click
        ...
        ElseIf str条件 = "合格确认" Then    'DataView数据表中筛选数据集(类似SQL语句).
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If 查询条件.Text.Length = 0 Then  '如果是空值
            ...
            I01_检测试验信息_Load(Nothing, Nothing)
    ...
    Private Sub 新建_Click(sender As Object, e As EventArgs) Handles 新建.Click
        ...
        品名.SelectedIndex = 0  '默认选择第一项
        目的区分.SelectedIndex = 0
    ...
    Private Sub 添加_Click(sender As Object, e As EventArgs) Handles 添加.Click
       ...
       ("SELECT TOP 1 * FROM 检测试验信息 ORDER BY ID DESC", objConnection1th)  '存贮当前记录位置给变量  Save the current record position..
        ...
        objCommand.CommandText = "INSERT INTO 检测试验信息 " &
            "(ID, 报告单号, 报告日期, 客户, 供应商, 材质, 品名, 检验项目, 目的区分, 报告来源, 检测依据, 合格确认, 文件资料, 备注) " &
            "VALUES(@ID, @报告单号, @报告日期, @客户, @供应商, @材质, @品名, @检验项目, @目的区分, @报告来源, @检测依据, @合格确认, @文件资料, @备注)"
        objCommand.Parameters.AddWithValue("@ID", strID)          '指定参数写入值,下同.
        objCommand.Parameters.AddWithValue("@报告单号", 报告单号.Text)
        ...
        objCommand.Parameters.AddWithValue("@检验项目", 检验项目.Text)
        objCommand.Parameters.AddWithValue("@目的区分", 目的区分.Text)
        objCommand.Parameters.AddWithValue("@报告来源", 报告来源.Text)
        objCommand.Parameters.AddWithValue("@检测依据", 检测依据.Text) '试试可不可以删
        objCommand.Parameters.AddWithValue("@合格确认", 合格确认.Checked).DbType = DbType.Boolean '试试可不可以删
        objCommand.Parameters.AddWithValue("@文件资料", 文件资料.Text) '试试可不可以删
        ...
        I01_检测试验信息_Load(Nothing, Nothing)         '调用方法填充数据到指定字段及绑定控件  Fill the dataset and bind the fields..
    ...
    Private Sub 更新_Click(sender As Object, e As EventArgs) Handles 更新.Click
        ...
        objCommand.CommandText = "UPDATE 检测试验信息 " &
                "SET 报告单号 = @报告单号,报告日期 = @报告日期,客户 = @客户,供应商 = @供应商,材质 = @材质,
品名 = @品名,检验项目 = @检验项目,目的区分 = @目的区分,报告来源 = @报告来源,检测依据 = @检测依据,合格确认 = @合格确认,
文件资料 = @文件资料,备注 = @备注 WHERE ID = @ID"
        ...
        objCommand.Parameters.AddWithValue("@报告单号", 报告单号.Text) '转换类型.
        objCommand.Parameters.AddWithValue("@报告日期", 报告日期.Text).DbType = DbType.Date
        objCommand.Parameters.AddWithValue("@客户", 客户.Text)
        objCommand.Parameters.AddWithValue("@供应商", 供应商.Text)
        objCommand.Parameters.AddWithValue("@材质", 材质.Text)
        ...
        objCommand.Parameters.AddWithValue("@目的区分", 目的区分.Text)
        objCommand.Parameters.AddWithValue("@报告来源", 报告来源.Text)
        objCommand.Parameters.AddWithValue("@检测依据", 检测依据.Text)
        objCommand.Parameters.AddWithValue("@合格确认", 合格确认.Checked).DbType = DbType.Boolean  '转换类型.
        objCommand.Parameters.AddWithValue("@文件资料", 文件资料.Text)
        objCommand.Parameters.AddWithValue("@备注", 备注.Text)
        ...
        I01_检测试验信息_Load(Nothing, Nothing) '调用方法显示数据和绑定字段  Fill the DataSet and bind the fields..
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        objCommand.CommandText = "DELETE FROM 检测试验信息 " &
                "WHERE ID = @ID"
    ...
    Private Sub 退出_Click(sender As Object, e As EventArgs) Handles 退出.Click
        ...
        Globals.Ribbons.Ribbon1.btnTesting.Enabled = True
        Me.Close()
    End Sub
    ...
    Private Sub 供应商_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 供应商.SelectedIndexChanged
        'objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "发现过程" & " from " & "赔偿比例 ORDER BY 发现过程" '写入SQL语句
        objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "产品规格" & " from " & "供应商及品名 WHERE 供应商 = " & "'" & 供应商.Text & "'"  '写入SQL语句
        objDataSet1th = New DataSet()                        '数据适配器对象开始检索数据并填充到DataSet对象
        ...
        For inCounter = 0 To tb2019042701.Rows.Count - 1               '在表行数上循环
            品名.Items.Add(tb2019042701.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
        Next
    End Sub


    Private Sub 检验项目_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 检验项目.SelectedIndexChanged
        'objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "发现过程" & " from " & "赔偿比例 ORDER BY 发现过程" '写入SQL语句
        On Error Resume Next
        objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "标准规范要求" & " from " & "标准规范要求 WHERE 项目 = " & "'" & 检验项目.Text & "'"  '写入SQL语句
        objDataSet1th = New DataSet()                        '数据适配器对象开始检索数据并填充到DataSet对象
        objDataAdapter1th.Fill(objDataSet1th, "wpxx20191116")      'Fill方法的第二参数可以随便填,最好填相关的数据源表,方便理解.
        Dim tb191116 As DataTable = objDataSet1th.Tables("wpxx20191116") '声明一个表类型,并赋值给该变量.
        检测依据.Items.Clear()             '给组合框添加项目  'Add items to the combo box..
        '不良类型.Items.Add("外注不良") ： 不良类型.Items.Add("内部工程不良-人员") ： 不良类型.Items.Add("内部工程不良-条件")
        '不良类型.Items.Add("内部工程不良-设备") ： 不良类型.Items.Add("内部工程不良-工具") ： 不良类型.Items.Add("内部工程不良-其他")
        '不良类型.Items.Add("客户发现不良")
        For inCounter = 0 To tb191116.Rows.Count - 1               '在表行数上循环
            检测依据.Items.Add(tb191116.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
        Next


        '别删,另一种方式获取值和图片,今后可能非常有用.
        rtxtSpec.Text = Spec.ResourceManager.GetObject(检验项目.Text)
        picbDisplayPicture.Image = Spec.ResourceManager.GetObject(检验项目.Text & "图")
        If Spec.ResourceManager.GetObject(检验项目.Text) = Nothing Then rtxtSpec.Text = Spec.ResourceManager.GetObject("其他") : picbDisplayPicture.Image = Spec.ResourceManager.GetObject("其他图")


        'Select Case 检验项目.Text
        '      '"序列号", "姓名", "性别", "出生年月", "技术职称", "专业等级", "发证日期", "有效期至", "证件编号"
        '    Case "冲击功S48CG"
        '        'Resources.ResourceManager.GetObject("新罗图标")

        '        rtxtSpec.Text = Spec.ResourceManager.GetObject("冲击功S48CG")
        '        picbDisplayPicture.Image = Spec.冲击功S48CG图
        '    Case "拉伸S48CG"
        '        rtxtSpec.Text = Spec.拉伸S48CG
        '        picbDisplayPicture.Image = Spec.新罗图标
        '    Case Else
        '        rtxtSpec.Text = Spec.等待
        '        picbDisplayPicture.Image = Nothing
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        Dim strRecordPath As String = "\\192.168.3.250\Erpupgrade\王飞共享体系资料\4 三方检报告\", bytPosition As Byte, bytAfterNamePosition As Byte, strFileName As String
        ...
        If IsArray(objFileArray) Then
            ...
            文件资料.Text = arrFileArrayResetting(0)

            bytPosition = InStr(1, StrReverse(文件资料.Text), "\")   '计算文件名称前面的“\”的位置
            bytAfterNamePosition = InStr(1, StrReverse(文件资料.Text), ".")   '计算文件名称前面的“\”的位置
            strFileName = Mid(文件资料.Text, Len(文件资料.Text) - bytPosition + 2)
            ...
            System.IO.File.Copy(文件资料.Text, strRecordPath & strFileName)

            文件资料.Text = strRecordPath & strFileName
    ...
    Private Sub btnOpenFile_Click(sender As Object, e As EventArgs) Handles btnOpenFile.Click
        On Error Resume Next
        If InStr(文件资料.Text, "xls") > 0 Then     '如果是包含有xls后缀存储名
            xlapp.Workbooks.Open(文件资料.Text)     '打开EXCEL
        Else                                        '否则
            'Shell("winword.exe " & 存放位置.Text, vbMaximizedFocus)   'shell函数打开word文档程序    
            Shell("explorer.exe " & 文件资料.Text, vbMaximizedFocus)   '打开所有资源程序
        End If                  '结束语句
        If Err.Number <> 0 Then MsgBox("打开对应文件失败",, "提示")
    End Sub

    Private Sub btnResetting_Click(sender As Object, e As EventArgs) Handles btnResetting.Click
        查询条件.Text = ""
    End Sub
