...
Public Class J02_产品合格检信息确认SqlServerNew

    Dim objConnection1th As New SqlConnection("server=192.168.3.52,1433\SQLEXPRESS;database=合格检验系统确认NEW;User ID=sa;Password=85773627;")  '张一帆服务器
    'Dim objConnection1th As New SqlConnection("server=192.168.3.136;database=合格检验系统确认;;Integrated Security=True;")  '本地服务器
    '声明作用域为类级的对象,该对象建立了与数据库的连接,此时数据库为Access.
    'Dim objConnection1th As New sqlConnection _
    '  ("Provider=Microsoft.Ace.sql.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\文件管理.accdb")  '公司共享盘52,1433\SQLEXPRESS
    '声明作用域为类级的对象,该对象建立了与数据库的连接,此时数据库为Access.
    Dim strSharePath As String = "192.168.3.250\Erpupgrade\王飞共享体系资料\access\合格检验系统确认装配.accdb"
    'Dim strYiFangPath As String = "\\192.168.3.52\Users\进销存管理.accdb"ddd12
    Dim strMyHomerComputerPath As String = "E:\access\合格检验系统确认装配.accdb"
    Dim strMyCompanyComputerPath As String = "D:\6 总务\access\合格检验系统确认装配.accdb"
    'Dim objConnection1th As New sqlConnection _
    ...
    '  Dim objConnection1th As New sqlConnection _
    '("Provider=Microsoft.Ace.sql.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\合格检验系统确认.accdb")  '公司共享盘
    '("Provider=Microsoft.Ace.sql.12.0;Data Source=D:\2 笔记记录\0 过程信息管理笔记\不良品信息管理\不良品信息管理.accdb")  '三星笔记本
    '("Provider=Microsoft.Ace.sql.12.0;Data Source=F:\2 笔记记录\8 过程信息管理\不良品信息管理\不良品信息管理.accdb")  '家里台式机
    '("Provider=Microsoft.Ace.sql.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\不良品信息管理.accdb")  '公司共享盘
    '声明作用域为类级的对象,该对象用于从数据库中读取数据,并填充到DataSet对象中.
    '这个构造函数使我们不必写Adapter属性SelectCommand相关代码.已经加入相关参数(SQL语句)
    'Dim objDataAdapter As New sqlDataAdapter("SELECT 合格检信息.* FROM 合格检信息 ORDER BY 检查ID", objConnection1th)

    Dim objDataAdapter As SqlDataAdapter
    Dim strGetData As String = "SELECT 合格检信息.* FROM 合格检信息 ORDER BY 检查ID"
    Dim objDataAdapter1th As New SqlDataAdapter()  '该构造函数需要使用SelectCommand属性.用来填充履历卡数据的
    ...
    Private Sub J01_产品合格检信息确认_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        objDataAdapter = New SqlDataAdapter(strGetData, objConnection1th)
        ...
        grdAuthorTitles.Columns("备注说明").HeaderCell.Value = "IDTag & 备注说明" '重新设置列标题的值显示为"描述"
        ...
        '检验员.Items.Add("杨伟伟") ： 检验员.Items.Add("裴建华") ： 检验员.Items.Add("杭燊业") ： 检验员.Items.Add("宗礼清") ： 检验员.Items.Add("杨宇") ： 检验员.Items.Add("邵松")
        检验员.Items.Add("杨宇") ： 检验员.Items.Add("杨敏") ： 检验员.Items.Add("裴俊男") ： 检验员.Items.Add("王勇") ： 检验员.Items.Add("徐钻") ： 检验员.Items.Add("段理想")
        检验员.Items.Add("杭燊业") ： 检验员.Items.Add("宗礼清") ： 检验员.Items.Add("朱长干") ： 检验员.Items.Add("陈晨") ： 检验员.Items.Add("张超")
    ...
    Private Sub 执行查询_Click(sender As Object, e As EventArgs) Handles 执行查询.Click
        ...
        If str条件 = "产品编号" And 类型区分.Text <> "Assembly" Then
        ...
        ElseIf str条件 = "产品编号" And 类型区分.Text = "Assembly" Then
            strGetData = "SELECT 合格检信息.* FROM 合格检信息 ORDER BY 检查ID"
            J01_产品合格检信息确认_Load(Nothing, Nothing)
            'strGetData = "SELECT 合格检信息.* FROM 合格检信息 WHERE 产品编号 = '" & strSplitData(0) & "' OR 产品编号 = '" & strSplitData(1) & "'  OR 产品编号 = '" & strSplitData(2) & "' ORDER BY 检查ID"
            strGetData = "SELECT 合格检信息.* FROM 合格检信息 WHERE 产品编号 LIKE '%" & strSplitData(0) & "%' OR 产品编号 = '" & strSplitData(1) & "'  OR 产品编号 = '" & strSplitData(2) & "' ORDER BY 检查ID"

        ElseIf str条件 = "检查日期" Then
            strGetData = "SELECT 合格检信息.* FROM 合格检信息 ORDER BY 检查ID"

            J01_产品合格检信息确认_Load(Nothing, Nothing)
            'str条件 & "='" & CType(查询条件.Text, Date).ToShortDateString & "'" '"Date = #12/31/2008 16:44:58#"
            strGetData = "SELECT 合格检信息.* FROM 合格检信息 WHERE " & str条件 & " = '" & CType(查询条件.Text, Date).ToShortDateString & "' ORDER BY 检查ID"
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If UCase(查询条件.Text) = "DELETE" Then 删除.Enabled = True : 更新.Enabled = True
    ...
    Private Sub 添加_Click(sender As Object, e As EventArgs) Handles 添加.Click
        ...
        'SUBSTR(colume1,0,1),length(colume1),colume1
        '("SELECT TOP 1 * FROM 合格检信息 ORDER BY 检查ID DESC", objConnection1th)  '存贮当前记录位置给变量  Save the current record position..
        ...
        ' 查询条件.Text = 产品编号.Text 'TEST for xuzuan requirement
        'strGetData = "SELECT 合格检信息.* FROM 合格检信息 ORDER BY 检查ID"
        'J01_产品合格检信息确认_Load(Nothing, Nothing)


        '根据选定的项并设置DataView对象(源数据是指定表sbxx)相关字段的sort属性.
        If ckId查看.Checked Then      'Determine the appropriate item selected and set the Sort property of the DataView object..            
    ...
    '    Dim b As String = 产品编号.Text, c As String = 类型区分.Text, d As Date = Now, e As String = ""
...
路径 = @路径,备注说明 = @备注说明 WHERE 检查ID = @检查ID"
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        'objCommand.CommandText = "DELETE FROM 工序信息 " &
        '        "WHERE 项目 <> 'Assembly'"
    ...
    Private Sub 退出_Click(sender As Object, e As EventArgs) Handles 退出.Click
        ...
        Globals.Ribbons.Ribbon1.btnQcCheckedNew.Enabled = True
        Me.Close()
    End Sub
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        If objFso.FolderExists("C:\MakeQRE") = True Then '如果存在指定的文件夹,那么执行
            'objFso.DeleteFolder("C:\MakeQRE")             '删除名为MakeQRE的文件夹
    ...
    Private Sub btnOpenFile_Click(sender As Object, e As EventArgs) Handles btnOpenFile.Click
        ...
        xlapp.ActiveSheet.Pictures.Insert(strShareStorge & strSplitData(0).ToString & "_" & 工序ID.Text & ".bmp").Select
    ...
    'Private Sub 产品编号_GotFocus(sender As Object, e As EventArgs)
    '    If 类型区分.Text = "Assembly" Then
    '        产品编号.Mask = "\A\ssebly\\AAAAAAAAAA\\IN\\AAAAAAAAA\\OT\\AAAAAAAAA"
    '    Else
    ...
    'Private Sub 产品编号_LostFocus(sender As Object, e As EventArgs)
    ...
    '    xlapp.ActiveSheet.Pictures.Insert("\\192.168.3.250\Erpupgrade\王飞共享体系资料\2 MakeQRE\FIcon.ico").Select
    '    xlapp.Selection.ShapeRange.Left = intShapLeft + (intShapWide / 2 - 6)     '移动图片与选择的单元格左边距离
    '    xlapp.Selection.ShapeRange.Top = intShapTop + (intShapHeight / 2 - 6)       '移动图片与选择的单元格上部距离相等
    ...
    Private Sub btnImportFile_Click(sender As Object, e As EventArgs) Handles btnImportFile.Click
        ...
        Dim strRecordPath As String = "\\192.168.3.12\DataBaseRecord\3 Record\", bytPosition As Byte, bytAfterNamePosition As Byte, strFileName As String
        ...
        If IsArray(objFileArray) Then
            ...
            路径.Text = arrFileArrayResetting(0)
            bytPosition = InStr(1, StrReverse(路径.Text), "\")   '计算文件名称前面的“\”的位置
            bytAfterNamePosition = InStr(1, StrReverse(路径.Text), ".")   '计算文件名称前面的“\”的位置
            strFileName = Mid(路径.Text, Len(路径.Text) - bytPosition + 2)
            ...
            System.IO.File.Copy(路径.Text, strRecordPath & strFileName)

            路径.Text = strRecordPath & strFileName
    ...
    Private Sub btnExportFile_Click(sender As Object, e As EventArgs) Handles btnExportFile.Click
        On Error Resume Next
        If InStr(路径.Text, "xls") > 0 Then     '如果是包含有xls后缀存储名
            xlapp.Workbooks.Open(路径.Text)     '打开EXCEL
            xlapp.ActiveWorkbook.Sheets(1).activate

        Else                                        '否则
            'Shell("winword.exe " & 存放位置.Text, vbMaximizedFocus)   'shell函数打开word文档程序    
            Shell("explorer.exe " & 路径.Text, vbMaximizedFocus)   '打开所有资源程序
        End If                  '结束语句
        If Err.Number <> 0 Then MsgBox("打开对应文件失败",, "提示")
    End Sub

    'Private Sub 检查ID_TextChanged(sender As Object, e As EventArgs) Handles 检查ID.TextChanged
    '    If 类型区分.Text <> "Assembly" Then PictureBox1.Image = Nothing : Exit Sub
    '    'xlapp.ActiveSheet.Shapes(bytShapesNumber).Name = Split(arrArray(0), ".")(0) & "."
    '    Dim strSplitData As String(), intCounter1 As Byte = 0, i As Byte = 0
    '    For i = 1 To 5 Step 2
    '        intCounter1 = intCounter1 + 1
    ...
    '        'xlapp.Range("a1").Value = Trim(产品编号.Text)
    '    Next
    '    If My.Computer.FileSystem.FileExists(myCompanyStorge & strSplitData(0).ToString & "_" & 工序ID.Text & ".bmp") Then
    '        PictureBox1.Image = Image.FromFile(myCompanyStorge & strSplitData(0).ToString & "_" & 工序ID.Text & ".bmp")
    '    Else
    '        Exit Sub
    '    End If
    ...
    'Private Sub 检查区域_TextChanged(sender As Object, e As EventArgs) Handles 检查区域.TextChanged
    '    If 类型区分.Text <> "Assembly" Then PictureBox1.Image = Nothing : Exit Sub
    '    'xlapp.ActiveSheet.Shapes(bytShapesNumber).Name = Split(arrArray(0), ".")(0) & "."
    '    Dim strSplitData As String(), intCounter1 As Byte = 0, i As Byte = 0
    '    For i = 1 To 5 Step 2
    ...
    '' 20200123 更新自动显示二维码功能
    'Private Sub 产品编号_TextChanged(sender As Object, e As EventArgs) Handles 产品编号.TextChanged
    '    On Error Resume Next
    '    PauseWait(2000)
    '    If 类型区分.Text <> "Assembly" Then PictureBox1.Image = Nothing : Exit Sub
    '    'xlapp.ActiveSheet.Shapes(bytShapesNumber).Name = Split(arrArray(0), ".")(0) & "."
    ...
    Private Sub 产品编号_GotFocus(sender As Object, e As EventArgs) Handles 产品编号.GotFocus
        If Len(产品编号.Text) > 20 Then
            产品编号.Text = 产品编号.Text
        ElseIf 类型区分.Text = "Assembly" Then
            产品编号.Mask = "\A\ssembly\\AAAAAAAAAAAA\\OT\\AAAAAAAAA\\IN\\AAAAAAAAA"
        Else
            产品编号.Mask = ""
        End If
    End Sub
    ...
    Private Sub btnClearPath_Click(sender As Object, e As EventArgs) Handles btnClearPath.Click
        'On Error Resume Next
        Kill(路径.Text)
        路径.Text = ""
    ...
    Private Sub J02_产品合格检信息确认SqlServerNew_MouseMove(sender As Object, e As MouseEventArgs) Handles Me.MouseMove
        On Error Resume Next
        Dim b As Long
        If e.Button = MouseButtons.Left Then
            With GroupBox4
                ...
                GroupBox3.Left = e.X + GroupBox3.Left - (GroupBox4.Left + b)
                GroupBox3.Width = GroupBox3.Width - (.Width - b)
                'GroupBox4.Size
            End With
        End If
    ...
    'Private Sub 检查ID_TextChanged(sender As Object, e As EventArgs) Handles 检查ID.TextChanged
    '    txtChangeDisplay.Text = 产品编号.Text
    'End Sub
    ...
    Private Sub txtChangeDisplay_TextChanged(sender As Object, e As EventArgs) Handles txtChangeDisplay.TextChanged
    ...
    Sub CallTheCodePicture()
        On Error Resume Next
        Dim strNumberProduct As String
        'PauseWait(2000)
        strNumberProduct = 产品编号.Text
        If 类型区分.Text <> "Assembly" Then PictureBox1.Image = Nothing : Exit Sub
        'xlapp.ActiveSheet.Shapes(bytShapesNumber).Name = Split(arrArray(0), ".")(0) & "."
        Dim strSplitData As String(), intCounter1 As Byte = 0, i As Byte = 0
        ...
        For i = 1 To 5 Step 2
            intCounter1 = intCounter1 + 1
            ReDim Preserve strSplitData(0 To intCounter1 - 1)   '重置一维数组上标
            strSplitData(intCounter1 - 1) = Trim(Split(strNumberProduct, "\")(i))  '在一维数组中写入值 
            'ReDim Preserve strSplitData(0 To )
            'strSplitData() = Split(产品编号.Text, "\")(i - 1)
            'xlapp.Range("a" & i).Value = Trim(s)
            'xlapp.Range("a1").Value = Trim(产品编号.Text)
    ...
    Private Sub btnOpenRev_Click(sender As Object, e As EventArgs) Handles btnOpenRev.Click
        On Error Resume Next
        Dim strPath As String = "\\192.168.3.12\DataBaseRecord\3 Record\服务器最新版本\", strFullname As String = ""
        Dim strFolderPath As String
        strFullname = Dir(strPath & "*.xl*")
        Do While strFullname <> ""


            If InStr(strFullname, 型号.Text & 类型区分.Text) Then

                xlapp.Workbooks.Open(strPath & strFullname)     '打开EXCEL
                With xlapp.FileDialog(4)
                    If .Show = -1 Then

                        strFolderPath = .SelectedItems(0) & IIf(Strings.Right(.SelectedItems(0), 1) = "\", "", "\")
                        xlapp.ActiveWorkbook.SaveAs(strFolderPath & strFullname)
    ...
    Private Sub btnKuoZhan_Click(sender As Object, e As EventArgs) Handles btnKuoZhan.Click
        Dim j As Integer, strSlectRng As String   '声明变量
        Dim rngRangelArea As Excel.Range  '这里跟VBA不一样,VBA是VBmsgboxresult
        Dim arr(,) As Object, arr2() As String         '声明变量
        RemoveHandler 产品编号.GotFocus, AddressOf 产品编号_GotFocus   '解除事件
        RemoveHandler 产品编号.TextChanged, AddressOf 产品编号_TextChanged   '解除事件
        RemoveHandler txtChangeDisplay.TextChanged, AddressOf txtChangeDisplay_TextChanged   '解除事件
        RemoveHandler 检查日期.TextChanged, AddressOf 检查日期_TextChanged   '解除事件
        RemoveHandler 检查日期.LostFocus, AddressOf 检查日期_LostFocus   '解除事件
        strSlectRng = xlapp.Selection.Address                                             '单元格地址赋值给文本变量
        rngRangelArea = xlapp.InputBox("请选择区域", "产品编码", strSlectRng, , , , , 8)  '单元格赋值给单元格对象变量
        产品编号.Mask = ""

        arr = rngRangelArea.Value          '将活动工作表中已用数据区域的值赋予变量
        ReDim arr2(0 To 8)
        arr2 = {客户.Text, 型号.Text, 类型区分.Text, 工序ID.Text, 项目.Text, 检查区域.Text, 检查日期.Text,
             检验员.Text, 路径.Text}
        For j = 1 To UBound(arr, 2)                                       '遍历数组的每一列(标题列例外)
            '如果成绩值小于60
            '累加计数器，此变量的值等于符合条件的成绩个数
            'Assembly\        \IN\         \OT\
            '重置数组变量的维数和上标(维数只能重置一次，最末维的上标则不限次数)
            'arr2(j - 1) = "Assembly\" & arr(1, j) & "\IN\" & arr(2, j) & "\OT\" & arr(3, j)     '将符合条件的姓名赋值到数组arr2的第1行中
            产品编号.Focus()
            产品编号.Text = ""
            ...
            产品编号.Text = "Assembly\" & arr(1, j) & "\IN\" & arr(2, j) & "\OT\" & arr(3, j)     '将符合条件的姓名赋值到数组arr2的第1行中
            客户.Text = 客户.Text
            添加_Click(Nothing, Nothing)
            'PauseWait(5000)
        Next j
        ...
        AddHandler 产品编号.GotFocus, AddressOf 产品编号_GotFocus      '绑定事件
        AddHandler txtChangeDisplay.TextChanged, AddressOf txtChangeDisplay_TextChanged   '绑定事件
        AddHandler 产品编号.TextChanged, AddressOf 产品编号_TextChanged   '解除事件
        AddHandler 检查日期.TextChanged, AddressOf 检查日期_TextChanged   '解除事件
        AddHandler 检查日期.LostFocus, AddressOf 检查日期_LostFocus   '解除事件
