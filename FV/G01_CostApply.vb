...
Public Class G01_CostApply
    ...
    '("Provider=Microsoft.Ace.OleDb.12.0;Data Source=D:\2 笔记记录\0 过程信息管理笔记\费用管理\费用管理.accdb") '三星笔记本
    '("Provider=Microsoft.Ace.OleDb.12.0;Data Source=F:\2 笔记记录\8 过程信息管理\费用申请\费用管理.accdb")  '家里台式机
    '("Provider=Microsoft.Ace.OleDb.12.0;Data Source=\\192.168.3.250\Erpupgrade\王飞共享体系资料\access\费用管理.accdb")  '公司共享盘
    '声明作用域为类级的对象,该对象用于从数据库中读取数据,并填充到DataSet对象中.
    '这个构造函数使我们不必写Adapter属性SelectCommand相关代码.已经加入相关参数(SQL语句)
    Dim objDataAdapter As New OleDbDataAdapter("SELECT 费用申请.* FROM 费用申请 ORDER BY 序列号", objConnection2th)
    ...
    Private Sub FillDataSetAndView()
        ...
        objDataAdapter.Fill(objDataSet, "fy")                 '表(zs)是初始构建起来的,命名为zs.
        objDataView = New DataView(objDataSet.Tables("fy"))   '初始化并构建DataView对象.
    ...
    Private Sub BindFields()
        ...
        myArray = {"序列号", "申请日期", "申请人", "项目申请", "用途", "数量", "适用单位", "总价", "完成状态", "备注"}
        ...
        For i = 0 To UBound(myArray)
            ...
            If GroupBox1.Controls(myArray(i).ToString).Name = "申请日期" Then GroupBox1.Controls(myArray(i).ToString).Text _
    ...
    Private Sub ShowPosition()
        Try                                                                                        '格式化日期指定短日期格式.
            申请日期.Text = Format(CType(GroupBox1.Controls("申请日期").Text, Date), "yyyy/MM/dd") '定义格式
        Catch e As System.Exception                                                                '声明一个错误变量类型
            GroupBox1.Controls("申请日期").Text = CType(Now, String)    '如果异常(文本框为空),那么转换当前日期类型为文本类型,并写入文本框中.
            申请日期.Text = Format(CType(GroupBox1.Controls("申请日期").Text, Date), "yyyy/MM/dd")  '重新转换Date类型.
        End Try
    ...
    '加载窗体触发事件
    Private Sub G01_CostApply_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        grdAuthorTitles.DataMember = "fy"           '设置gird控件要显示的数据源(具体的表名称).
        ...
        grdAuthorTitles.Columns(0).Width = 70 '设置指定列默认宽度大一点
        grdAuthorTitles.Columns(1).HeaderText = "申请日期"
        grdAuthorTitles.Columns(2).HeaderText = "申请人"
        grdAuthorTitles.Columns(2).Width = 80 '设置指定列默认宽度大一点
        grdAuthorTitles.Columns(3).HeaderText = "项目申请"
        grdAuthorTitles.Columns(4).HeaderText = "用途"
        grdAuthorTitles.Columns(4).Width = 170 '设置指定列默认宽度大一点
        grdAuthorTitles.Columns(5).HeaderText = "数量"
        grdAuthorTitles.Columns(5).Width = 35 '设置指定列默认宽度大一点
        grdAuthorTitles.Columns(6).HeaderText = "适用单位"
        grdAuthorTitles.Columns(6).Width = 35 '设置指定列默认宽度大一点
        grdAuthorTitles.Columns(7).HeaderText = "总价"
        grdAuthorTitles.Columns(8).HeaderText = "完成状态"
        grdAuthorTitles.Columns(9).HeaderText = "备注"
        grdAuthorTitles.Columns(9).Width = 300 '设置指定列默认宽度大一点
        ...
        For i As Integer = 0 To grdAuthorTitles.RowCount - 1                           '有一个空白行也算一行
            'If Math.Ceiling(CType(grdAuthorTitles.Item(1, i).Value.ToString(), Date).Subtract(Now).TotalDays) <= 3 Then
            'If -Math.Ceiling(CType(grdAuthorTitles.Item(1, i).Value.ToString(), Date).Subtract(Now).TotalDays) >= 3 And grdAuthorTitles.Item(8, i).Value.ToString() <> "完成" Then
            If grdAuthorTitles.Item(8, i).Value.ToString() <> "完成" Then
        ...
        适用单位.Items.Clear()             '给组合框添加项目  'Add items to the combo box..
        适用单位.Items.Add("个")
        适用单位.Items.Add("套")
        适用单位.Items.Add("组")
        适用单位.Items.Add("批次")
        适用单位.Items.Add("其他")
        适用单位.Items.Add("次")   '维修类型.SelectedIndex = 0    '默认选择第一项
        BindFields() '调用绑定控件过程
    ...
    Private Sub 执行排序_Click(sender As Object, e As EventArgs) Handles 执行排序.Click
        ...
        Select Case 排序字段.SelectedIndex      'Determine the appropriate item selected and set the Sort property of the DataView object..            
            ...
            Case 1
                objDataView.Sort = "申请日期"
            Case 2
                objDataView.Sort = "申请人"
            Case 3
                objDataView.Sort = "项目申请"
            Case 4
                objDataView.Sort = "用途"
            Case 5
                objDataView.Sort = "数量"
            Case 6
                objDataView.Sort = "适用单位"
            Case 7
                objDataView.Sort = "总价"
            Case 8
                objDataView.Sort = "完成状态"
            Case 9
                objDataView.Sort = "备注"
        End Select
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If 查询条件.Text.Length = 0 Then              '如果是空值
            G01_CostApply_Load(Nothing, Nothing)   '调用加载窗体事件.填充数据显示DateGirdVie完整视图,绑定控件,显示当前记录位置
        End If
    End Sub
    ...
    Private Sub 添加_Click(sender As Object, e As EventArgs) Handles 添加.Click
       ...
       ("SELECT TOP 1 * FROM 费用申请 ORDER BY 序列号 DESC", objConnection2th)  '存贮当前记录位置给变量  Save the current record position..
        objConnection2th.Open()   '打开数据库连接 Open the connection, execute the command SELECT TOP 1 * FROM 表名 ORDER BY 排序字段 DESC
        ...
        Else
            ...
            intMaxID = CType(strID.Remove(0, 2), Integer)   '利用Remove方法删除xl前缀,转换整型赋值给变量intMaxID.Get the integer part of the string..
        ...
        End Select
        objCommand.Connection = objConnection2th '设置命令对象的属性 Set the SqlCommand object properties..'将连接字符串的连接对象赋值给Connection属性
        '维修单号.Enabled = True'将CommandText属性(要执行的SQL字符串)设置指定的值
        'myArray = {"序列号", "姓名", "性别", "出生年月", "技术职称", "专业等级", "发证日期", "有效期至", "证件编号"}
        objCommand.CommandText = "INSERT INTO 费用申请 " &
        "(序列号, 申请日期, 申请人, 项目申请, 用途, 数量, 适用单位, 总价, 完成状态, 备注) " &
        "VALUES(@序列号, @申请日期, @申请人, @项目申请, @用途, @数量, @适用单位, @总价, @完成状态, @备注)"
        ...
        objCommand.Parameters.AddWithValue("@申请日期", 申请日期.Text).DbType = DbType.Date
        objCommand.Parameters.AddWithValue("@申请人", 申请人.Text)
        objCommand.Parameters.AddWithValue("@项目申请", 项目申请.Text) '转换日期类型
        objCommand.Parameters.AddWithValue("@用途", 用途.Text)
        objCommand.Parameters.AddWithValue("@数量", 数量.Text)
        objCommand.Parameters.AddWithValue("@适用单位", 适用单位.Text)
        objCommand.Parameters.AddWithValue("@总价", 总价.Text)                                                           '转
        objCommand.Parameters.AddWithValue("@完成状态", 完成状态.Text)
        ...
        End Try                                         '结束截取
        objConnection2th.Close()                        '关闭数据库连接 Close the connection..
        G01_CostApply_Load(Nothing, Nothing)         '调用方法填充数据到指定字段及绑定控件  Fill the dataset and bind the fields..
    ...
    Private Sub 更新_Click(sender As Object, e As EventArgs) Handles 更新.Click
        ...
        objCommand.Connection = objConnection2th '设置命令对象一些属性 Set the SqlCommand object properties..
        'SQL语句表示按照指定条件,更新表设备名称  "放置地点", "制造商", "制造日期", "使用部门", "运行状态"等
        'myArray = {"序列号", "姓名", "性别", "出生年月", "技术职称", "专业等级", "发证日期", "有效期至", "证件编号"}
        objCommand.CommandText = "UPDATE 费用申请 " &
            "SET 申请日期 = @申请日期,申请人 = @申请人,项目申请 = @项目申请,用途 = @用途,数量 = @数量,适用单位 = @适用单位,总价 = @总价,完成状态 = @完成状态,备注 = @备注 WHERE 序列号 = @序列号"  ' '接着使用SQL字符串设置CommandText属性.
        ...
        objCommand.Parameters.AddWithValue("@申请日期", 申请日期.Text).DbType = DbType.Date  '转换类型.
        objCommand.Parameters.AddWithValue("@申请人", 申请人.Text)
        objCommand.Parameters.AddWithValue("@项目申请", 项目申请.Text)
        objCommand.Parameters.AddWithValue("@用途", 用途.Text)
        objCommand.Parameters.AddWithValue("@数量", 数量.Text)
        objCommand.Parameters.AddWithValue("@适用单位", 适用单位.Text)
        objCommand.Parameters.AddWithValue("@总价", 总价.Text)
        objCommand.Parameters.AddWithValue("@完成状态", 完成状态.Text)
        objCommand.Parameters.AddWithValue("@备注", BindingContext(objDataView).Current("备注"))
        objCommand.Parameters.AddWithValue("@序列号", 序列号.Text)
        objConnection2th.Open()                 '打开带连接字符的数据库连接  Open the connection..
        objCommand.ExecuteNonQuery()            '执行命令对象以更新数据 Execute the SqlCommand object to update the data..
        objConnection2th.Close()                '关闭数据库连接  Close the connection..
        G01_CostApply_Load(Nothing, Nothing) '调用方法显示数据和绑定字段  Fill the DataSet and bind the fields..
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        If intPosition < 0 Then  '如果没有记录,则设置记录位置为o.    If the position is less than 0 set it to 0..
            intPosition = 0
        End If
        objCommand.Connection = objConnection2th      '设置命令对象属性 Set the Command object properties..
        objCommand.CommandText = "DELETE FROM 费用申请 " &
            "WHERE 序列号 = @序列号"
        ...
        objConnection2th.Open()     '打开数据库连接 Open the database connection..
        ...
        G01_CostApply_Load(Nothing, Nothing)
    ...
    '关闭
    Private Sub G01_CostApply_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        '清理内存及数据适配器对象
        objDataAdapter = Nothing           '清理数据适配器对象,释放内存 ' Clean up
        objConnection2th = Nothing         '清理连接对象,释放内存
        Globals.Ribbons.Ribbon1.btnCost.Enabled = True
    End Sub
