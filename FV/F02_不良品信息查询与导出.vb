...
Public Class F02_不良品信息查询与导出
    ...
    Private Sub F02_不良品信息查询与导出_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        '为信息种类复合框设置项目
        With 信息种类       '引用复合框对象
            .Items.Add（"不良品信息"）     '添加项目为合同基本信息
        End With                                '结束引用语句
    ...
    Private Sub 开始查询_Click(sender As Object, e As EventArgs) Handles 开始查询.Click
        ...
        ElseIf 查询项目.Text = "处置完成" Then
            Con1 = CType(条件值1.Text, Boolean)
        Else        '否则给变量赋值
    ...
    Private Sub 数据导出_Click(sender As Object, e As EventArgs) Handles 数据导出.Click
        ...
        'objDataAdapter1th.SelectCommand = New OleDbCommand()
        'objDataAdapter1th.SelectCommand.Connection = objConnection2th
        'objDataAdapter1th.SelectCommand.CommandText = "select * from " & 信息种类.Text & " where 不良数量 > 0"
        'objDataAdapter1th.Fill(objDataSet1th, "wxxx4")
        Dim tb As DataTable = objDataSet1th.Tables("wxxx3")
        'objDataAdapter1th.SelectCommand.CommandType = CommandType.Text
        'grdAuthorTitles1th.DataSource = objDataSet1th
        'grdAuthorTitles1th.AutoGenerateColumns = True
        ...
        With ws                     '引用工作表
            ...
            For i = 1 To tb.Rows.Count    '在1到记录数量上循环
                ReDim Preserve objArray(a, 0 To i - 1)
                For j = 0 To a
                    ...
                    If tb.Rows(i - 1).Item(j).GetType.ToString = "System.DateTime" Then      '如果字段值类型是日期
                        ...
                        objArray(j, i - 1) = tb.Rows(i - 1).Item(j).ToString
                        '.Cells(i + 1, j + 1) = tb.Rows(i - 1).Item(j).ToString
                    ElseIf tb.Rows(i - 1).Item(j).GetType.ToString = "System.Boolean" Then
                        If tb.Rows(i - 1).Item(j).ToString = True Then
                            objArray(j, i - 1) = "处置完成"
                            '.Cells(i + 1, j + 1) = "处置完成"
                        Else
                            objArray(j, i - 1) = "未完成"
                    ...
                    Else
                        ...
                        objArray(j, i - 1) = tb.Rows(i - 1).Item(j).ToString '在单元格上写入字段值
            ...
            xlapp.Cells(2, 1).Resize(tb.Rows.Count, tb.Columns.Count).Value = xlapp.WorksheetFunction.Transpose(objArray)
            .Columns.AutoFit()    '列标自动设置适应宽度
        End With        '结束引用


        '插入数据透视图
        If xlapp.Sheets.Count < 2 Then xlapp.Sheets.Add() Else xlapp.Sheets("Sheet3").Select : xlapp.Sheets("Sheet3").Delete

        'C19表示第19列
        xlapp.ActiveWorkbook.PivotCaches.Create(SourceType:=1, SourceData:=
        "Sheet1!R1C1:R" & xlapp.Worksheets("Sheet1").Range("a1").CurrentRegion.Rows.Count & "C19", Version:=4).CreatePivotTable _
        (TableDestination:="Sheet2!R1C1", TableName:="数据透视表1", DefaultVersion:=
        4)
        ...
        xlapp.ActiveChart.SetSourceData(Source:=xlapp.Range("Sheet2!$A$1:$C$19"))
        xlapp.ActiveSheet.Shapes(1).IncrementLeft(216)
        xlapp.ActiveSheet.Shapes(1).IncrementTop(13.5)
        With xlapp.ActiveSheet.PivotTables("数据透视表1").PivotFields("不良类型")
            .Orientation = 3
            .Position = 1
        End With
        With xlapp.ActiveSheet.PivotTables("数据透视表1").PivotFields("完成工序")
            .Orientation = 1
            .Position = 1
        End With
        xlapp.ActiveSheet.PivotTables("数据透视表1").AddDataField(xlapp.ActiveSheet.PivotTables("数据透视表1"
        ).PivotFields("不良数量"), "求和项:不良数量", -4157)
        xlapp.ActiveSheet.PivotTables("数据透视表1").AddDataField(xlapp.ActiveSheet.PivotTables("数据透视表1"
        ).PivotFields("材料费用"), "求和项:材料费用", -4157)
        xlapp.ActiveSheet.PivotTables("数据透视表1").AddDataField(xlapp.ActiveSheet.PivotTables("数据透视表1"
        ).PivotFields("加工费用"), "求和项:加工费用", -4157)
        xlapp.ActiveSheet.ChartObjects(1).Activate
        xlapp.ActiveChart.ChartType = 5
        xlapp.ActiveChart.SeriesCollection(1).Select
        xlapp.ActiveChart.SetElement(210)
        '添加行标题
        xlapp.ActiveSheet.PivotTables("数据透视表1").CompactLayoutRowHeader = "工序"
        xlapp.ActiveSheet.PivotTables("数据透视表1").DataPivotField.PivotItems("求和项:不良数量").
        Caption = "     不良数量"
        'xlapp.Range("C4").Select()
        xlapp.ActiveSheet.PivotTables("数据透视表1").DataPivotField.PivotItems("求和项:材料费用").
        Caption = "     材料费用"
        'xlapp.Range("D4").Select()
        xlapp.ActiveSheet.PivotTables("数据透视表1").DataPivotField.PivotItems("求和项:加工费用").
        Caption = "     加工费用"
        ...
        rngTargetRange = xlapp.Range("a1000").End(-4162).Offset(2)



        xlapp.ActiveSheet.Shapes(1).ScaleWidth(1.6135415573, 0,
        2)
        xlapp.ActiveSheet.Shapes(1).ScaleHeight(1.7361111111, 0,
        0)
        'xlapp.ActiveSheet.Shapes(1).IncrementLeft(rngTargetRange.Left)
        'xlapp.ActiveSheet.Shapes(1).IncrementLeft(0)
        'xlapp.ActiveSheet.Shapes(1).IncrementTop(rngTargetRange.Top)

        xlapp.ActiveSheet.Shapes(1).Left = rngTargetRange.Left + 0.7       '移动图片与选择的单元格左边距离
        xlapp.ActiveSheet.Shapes(1).Top = rngTargetRange.Top + 0.7         '移动图片与选择的单元格上部距离相等



        'ws.ListObjects.Add(1, xlapp.Range("a1").CurrentRegion,, 1).Name =
        '"表1"
        ...
        ws.ListObjects("表4").Sort.SortFields.Add(xlapp.Range("b2"), 0, 1, 0)
        ...
        For i = 2 To tb.Rows.Count + 1 '数据记录数量+1 表示的是加上标题行为最后一行对应的Excel的行数
            '在B列创建超级链接,从而允许单击单元格时进入相应的工作表
            If xlapp.Cells(i, 21).value <> "" Then
                ws.Hyperlinks.Add(xlapp.Cells(i, 21), xlapp.Cells(i, 21).value.ToString)
                xlapp.Cells(i, 21).value = "Click here"
            End If
        Next
        xlapp.Columns("U:U").ColumnWidth = 15 '指定列宽缩小
        With ws.ListObjects("表4")
            .ShowTotals = True
            .ListColumns("不良数量").TotalsCalculation = 1
        End With
        xlapp.Sheets("Sheet1").Name = "详单"
        xlapp.Sheets("Sheet2").Name = "统计表"
        'xlapp.Cells.Select()
        'xlapp.Selection.RowHeight = 16.5
        ws = Nothing        '释放变量
        wb = Nothing
        关闭窗体.PerformClick()
        关闭窗体_Click(Nothing, Nothing)
        xlapp.ScreenUpdating = True  '关闭屏幕更新
    End Sub
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select * from 不良品信息"
        objDataAdapter1th.Fill(objDataSet1th, "wxxx4")
        'objDataView1th = New DataView(objDataSet1th.Tables("wxxx3"))
        Dim tbAllData As DataTable = objDataSet1th.Tables("wxxx4")
        ...
        For i = 2 To tb.Rows.Count + 1 '数据记录数量+1 表示的是加上标题行为最后一行对应的Excel的行数
            '在B列创建超级链接,从而允许单击单元格时进入相应的工作表
            'If xlapp.Cells(i, 12).value <> "" Then
            '    '在B列创建超级链接,从而允许单击单元格时进入相应的工作表
            '    ws.Hyperlinks.Add(xlapp.Cells(i, 12), xlapp.Cells(i, 12).value.ToString)
            '    xlapp.Cells(i, 12).value = "Click here"
            'End If
    ...
    Private Sub 关闭窗体_Click(sender As Object, e As EventArgs) Handles 关闭窗体.Click
        ...
        Globals.Ribbons.Ribbon1.btn不良品信息查询与导出.Enabled = True
        myArray = Nothing
        Me.Close()
    End Sub

    Private Sub F02_不良品信息查询与导出_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        关闭窗体_Click(Nothing, Nothing)
    End Sub
