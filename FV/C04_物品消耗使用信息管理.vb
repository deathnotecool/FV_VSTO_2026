...
Public Class C04_物品消耗使用信息管理
    ...
    Dim objDataAdapter As New OleDbDataAdapter("SELECT 物品消耗使用信息.* FROM 物品消耗使用信息 ORDER BY 消耗使用编号", objConnection)
    ...
    Private Sub FillDataSetAndView()
        ...
        objDataAdapter.Fill(objDataSet, "XHXX")  '这里没有设置SelectCommand属性,因为在初始化Adapter对象时,已经使用了相应的参数.
        '设置对应表为数据源绑定到DataView类  Set the DataView object to the DataSet object.
        objDataView = New DataView(objDataSet.Tables("XHXX"))
    ...
    Private Sub BindFields()
        ...
        myArray = {"消耗使用编号", "物品编码", "物品名称", "物品规格", "计量单位", "消耗使用数量", "成本单价", "出库使用日期", "备注"}
        ...
        For i = 0 To UBound(myArray)
            ...
            If GroupBox1.Controls(myArray(i).ToString).Name = "出库使用日期" Then GroupBox1.Controls(myArray(i).ToString).Text = Format(CType(GroupBox1.Controls(myArray(i).ToString).Text, Date), "yyyy/MM/dd")
            'GroupBox1.Controls(myArray(i).ToString).DataBindings.Add("Text", objDataView, GroupBox1.Controls(myArray(i).ToString).Name)
        Next i
    ...
    Private Sub ShowPosition()
        ...
        Try
            出库使用日期.Text = Format(CType(GroupBox1.Controls("出库使用日期").Text, Date), "yyyy/MM/dd") '定义格式
        Catch e As System.Exception
            GroupBox1.Controls("出库使用日期").Text = CType(Now, String)   '如果异常(文本框为空)那么将日期写为当前日期
            出库使用日期.Text = Format(CType(GroupBox1.Controls("出库使用日期").Text, Date), "yyyy/MM/dd")  '重新转换Date类型.
        End Try
    ...
    Private Sub C04_物品消耗使用信息管理_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        grdAuthorTitles.DataMember = "XHXX"  'gird控件要显示数据源(填充过数据的DataSet对象)具体的表名称
        ...
        'myArray = {"消耗使用编号", "物品编码", "物品名称", "物品规格", "计量单位", "消耗使用数量", "成本单价", "出库使用日期", "备注"}
        grdAuthorTitles.Columns(0).HeaderText = "消耗使用编号"
        grdAuthorTitles.Columns(1).HeaderText = "物品编码"
        grdAuthorTitles.Columns(2).HeaderText = "物品名称"
        grdAuthorTitles.Columns(3).HeaderText = "物品规格"
        grdAuthorTitles.Columns(4).HeaderText = "计量单位"
        grdAuthorTitles.Columns(5).HeaderText = "消耗使用数量"
        grdAuthorTitles.Columns(6).HeaderText = "成本单价"
        grdAuthorTitles.Columns(7).HeaderText = "出库使用日期"
        grdAuthorTitles.Columns(8).HeaderText = "备注"
        grdAuthorTitles.Columns(8).Width = 165 '设置指定列默认宽度小一点
        '改变字段标题名称和样式  
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "物品编码" & " from " & "采购物品信息" &
            " ORDER BY 物品编码"
        ...
        objDataAdapter1th.Fill(objDataSet1th, "xhxx2")
        'objDataView1th = New DataView(objDataSet.Tables("jhxx2"))
        Dim tb As DataTable = objDataSet1th.Tables("xhxx2")
        物品编码.Items.Clear()
        For inCounter = 0 To tb.Rows.Count - 1
            物品编码.Items.Add(tb.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
        Next
        'objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "进货单价" & " from " & "采购进货信息 " &
        '    "where 物品编码='" & 物品编码.Text & "'"
        'objDataAdapter1th.SelectCommand.CommandType = CommandType.Text
        'objDataAdapter1th.Fill(objDataSet1th, "xhxx5")
        'Dim tb1 As DataTable = objDataSet1th.Tables("xhxx5")
        '成本单价.Items.Clear()
    ...
    Private Sub 执行排序_Click(sender As Object, e As EventArgs) Handles 执行排序.Click
        ...
        Select Case 排序字段.SelectedIndex
            ...
            Case 5
                objDataView.Sort = "消耗使用数量"
            Case 6
                objDataView.Sort = "成本单价"
            Case 7
                objDataView.Sort = "出库使用日期"
            Case 8
                objDataView.Sort = "备注"
        End Select
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If 查询条件.Text.Length = 0 Then  '如果是空值
            ...
            C04_物品消耗使用信息管理_Load(Nothing, Nothing)   '调用加载窗体事件.填充数据显示DateGirdVie完整视图,绑定控件,显示当前记录位置
    ...
    Private Sub 添加_Click(sender As Object, e As EventArgs) Handles 添加.Click
       ...
       ("SELECT TOP 1 * FROM 物品消耗使用信息 ORDER BY 消耗使用编号 DESC", objConnection)
        ...
        '    Case 1
        '        strID = "XH00" & intMaxID.ToString
        '    Case 2
        '        strID = "XH0" & intMaxID.ToString
        'End Select
        ...
        ' myArray = {"消耗使用编号", "物品编码", "物品名称", "物品规格", "计量单位", "消耗使用数量", "成本单价", "出库使用日期", "备注"}
        objCommand.CommandText = "INSERT INTO 物品消耗使用信息 " &
        "(消耗使用编号, 物品编码, 物品名称, 物品规格, 计量单位, 消耗使用数量, 成本单价, 出库使用日期, 备注) " &
        "VALUES(@消耗使用编号, @物品编码, @物品名称, @物品规格, @计量单位, @消耗使用数量, @成本单价, @出库使用日期, @备注)"
        ...
        objCommand.Parameters.AddWithValue("@消耗使用编号", strID)
        ...
        objCommand.Parameters.AddWithValue("@消耗使用数量", 消耗使用数量.Text)      '.DbType = DbType.Int32
        objCommand.Parameters.AddWithValue("@成本单价", 成本单价.Text).DbType = DbType.Currency
        objCommand.Parameters.AddWithValue("@出库使用日期", 出库使用日期.Text).DbType = DbType.Date
        ...
        C04_物品消耗使用信息管理_Load(Nothing, Nothing)
    ...
    Private Sub 更新_Click(sender As Object, e As EventArgs) Handles 更新.Click
        ...
        ' myArray = {"消耗使用编号", "物品编码", "物品名称", "物品规格", "计量单位", "消耗使用数量", "成本单价", "出库使用日期", "备注"}
        objCommand.CommandText = "UPDATE 物品消耗使用信息 " &
            "SET 物品编码 = @物品编码,物品名称 = @物品名称,物品规格 = @物品规格,计量单位 = @计量单位,消耗使用数量 = @消耗使用数量,成本单价 = @成本单价,出库使用日期 = @出库使用日期,备注 = @备注 WHERE 消耗使用编号 = @消耗使用编号"
        ...
        objCommand.Parameters.AddWithValue("@物品规格", 物品规格.Text)
        objCommand.Parameters.AddWithValue("@计量单位", 计量单位.Text)
        objCommand.Parameters.AddWithValue("@消耗使用数量", 消耗使用数量.Text)
        objCommand.Parameters.AddWithValue("@成本单价", 成本单价.Text).DbType = DbType.Currency
        objCommand.Parameters.AddWithValue("@出库使用日期", 出库使用日期.Text).DbType = DbType.Date
        objCommand.Parameters.AddWithValue("@备注", 备注.Text)
        objCommand.Parameters.AddWithValue _
            ("@消耗使用编号", BindingContext(objDataView).Current("消耗使用编号"))
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        'myArray = {"消耗使用编号", "物品编码", "物品名称", "物品规格", "计量单位", "消耗使用数量", "成本单价", "出库使用日期", "备注"}
        '如果没有记录,则设置记录位置为0.    If the position is less than 0 set it to 0..
        If intPosition < 0 Then
            intPosition = 0
        End If
        '设置命令对象属性 Set the Command object properties..
        objCommand.Connection = objConnection
        objCommand.CommandText = "DELETE FROM 物品消耗使用信息 " &
            "WHERE 消耗使用编号 = @消耗使用编号"
    ...
    Private Sub 退出_Click(sender As Object, e As EventArgs) Handles 退出.Click
        ...
        Globals.Ribbons.Ribbon1.Button34.Enabled = True     '重新使按钮可用.
        Me.Close()  '关闭窗体
    End Sub

    Private Sub C04_物品消耗使用信息管理_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        '清理内存及数据适配器对象
        objDataAdapter = Nothing           '清理数据适配器对象,释放内存 ' Clean up
        objConnection = Nothing            '清理连接对象,释放内存
        Globals.Ribbons.Ribbon1.Button34.Enabled = True  '重新使按钮可用.
    ...
    Private Sub 物品编码_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 物品编码.SelectedIndexChanged
        ...
        objDataAdapter1th.SelectCommand = New OleDbCommand()
        objDataAdapter1th.SelectCommand.Connection = objConnection
        objDataAdapter1th.SelectCommand.CommandText = "select * from 采购物品信息 where 物品编码='" & 物品编码.Text & "'" &
        " ORDER BY 物品编码"
        '这里的SelectCommand的CommandType属性就是CommandType.Text,是默认属性可以省略的.
        objDataAdapter1th.SelectCommand.CommandType = CommandType.Text
        '数据适配器对象开始检索数据并填充到DataSet对象
        ...
        'Dim i As Byte
        物品名称.Text = tb.Rows(0).Item(1).ToString
        物品规格.Text = tb.Rows(0).Item(2).ToString
        计量单位.Text = tb.Rows(0).Item(3).ToString
        ...
        objDataAdapter1th.Fill(objDataSet1th, "xhxx5")
        Dim tb1 As DataTable = objDataSet1th.Tables("xhxx5")
        Dim k As Int16 = tb1.Columns.Count
        Dim k1 As Int16 = tb1.Rows.Count
        成本单价.Text = tb1.Rows(0).Item(0).ToString
        成本单价.Enabled = False
        '成本单价.Items.Clear()
        'For inCounter = 0 To tb1.Rows.Count - 1
        '    成本单价.Items.Add(tb1.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
    ...
    Private Sub 出库使用日期_GotFocus(sender As Object, e As EventArgs) Handles 出库使用日期.GotFocus
        出库使用日期.Mask = "0000/00/00"
    End Sub

    Private Sub 出库使用日期_LostFocus(sender As Object, e As EventArgs) Handles 出库使用日期.LostFocus
