...
Public Class B04_设备信息查询与导出
    ...
    Private Sub B04_设备信息查询与导出_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        '为信息种类复合框设置项目
        With 信息种类       '引用复合框对象
            .Items.Add（"设备名称"）     '添加项目为合同基本信息
            .Items.Add（"维修"）   '添加项目为合同收费信息
            .Items.Add（"保养"）   '添加项目为合同收费信息
        End With                                '结束引用语句
    ...
    Private Sub 查询项目_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 查询项目.SelectedIndexChanged
        ...
        With tb
            For inCounter = 0 To .Rows.Count - 1
                ...
                If 查询项目.Text = "制造日期" Or 查询项目.Text = "报修时间" Or 查询项目.Text = "保养时间" Then
    ...
    Private Sub 开始查询_Click(sender As Object, e As EventArgs) Handles 开始查询.Click
        ...
        '设置控件列标题      'Declare and set the currency header alignment property..               
        '初始化OleDbCommand类的一个实例,并将其分配给SelectCommand属性.   Set the SelectCommand properties..
        '将Connection属性设置为连接对象.用来与数据库通信.
        '设置选择命令字符串的CommandText属性设置为要要执行的SQL语句(也可以是存储过程)
    ...
    Private Sub B04_设备信息查询与导出_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        关闭窗体_Click(Nothing, Nothing)
    End Sub
