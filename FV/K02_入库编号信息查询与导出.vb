...
Public Class K02_入库编号信息查询与导出
    'Dim strYiFangPath As String = "\\192.168.3.52\Users\入库编号信息管理.accdb"
    ...
    'New关键字没有SQL执行的语句,说明Adapter调制器存的是空数据集.
    ...
    '声明变体型对象变量,赋值后自动成为数组变量...
    Dim myArray As Object


    Private Sub K02_入库编号信息查询与导出_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        '为信息种类复合框设置项目
        With 信息种类       '引用复合框对象
            .Items.Add（"入库编号信息"）     '添加项目为合同基本信息
        End With                                '结束引用语句
    ...
    Private Sub 数据导出_Click(sender As Object, e As EventArgs) Handles 数据导出.Click
        ...
        '    ws.Hyperlinks.Add(xlapp.Cells(i, 13), xlapp.Cells(i, 13).value.ToString)
        '    xlapp.Cells(i, 13).value = "Click here"
        'Next
        ...
        xlapp.Columns("M:M").ColumnWidth = 15 '指定列宽缩小
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select * from 入库编号信息"
        objDataAdapter1th.Fill(objDataSet1th, "wxxx4")
        'objDataView1th = New DataView(objDataSet1th.Tables("wxxx3"))
        ...
        ws.ListObjects.Add(1, xlapp.Range("a1").CurrentRegion,, 1).Name = "表4"
        ws.ListObjects("表4").Sort.SortFields.Clear()
        ws.ListObjects("表4").Sort.SortFields.Add(xlapp.Range("N2"), 0, 1, 0)
        With ws.ListObjects("表4").Sort
            .Header = 1
            .MatchCase = False
            .Orientation = 1
    ...
    Private Sub 关闭窗体_Click(sender As Object, e As EventArgs) Handles 关闭窗体.Click
        ...
        Globals.Ribbons.Ribbon1.btnLayoutCode.Enabled = True
        myArray = Nothing
        Me.Close()
    End Sub

    Private Sub I02_就爱你猜测试信息查询与导出_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        关闭窗体_Click(Nothing, Nothing)
    End Sub
