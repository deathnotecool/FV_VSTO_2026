...
Public Class F03_不良品发生分析
    ...
    Private Sub F03_不良品发生分析_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub 不良类型()
        ...
        objDataAdapter1th.SelectCommand.CommandText = "select distinct 不良类型 from 不良品信息  where 发生日期 between #" &
      起始日期.Value & "# and #" & 截止日期.Value & "#"
        ...
        Dim a As Integer = tb.Rows.Count - 1
        Dim i As Integer
        选择不良类型.Items.Clear()
        With 选择不良类型
            For i = 0 To a
            ...
            Next
        End With  '结束引用语句
        选择不良类型.SelectedIndex = 0      '选择默认项目为第一项
    End Sub
    Private Sub 选择不良类型_SelectedIndexChanged(sender As Object, e As EventArgs) Handles 选择不良类型.SelectedIndexChanged
        'On Error Resume Next        '错误时继续执行
        '初始化OleDbCommand类的一个实例,并将其分配给SelectCommand属性.   Set the SelectCommand properties..
        objDataAdapter1th.SelectCommand = New OleDbCommand()
        '将Connection属性设置为连接对象.用来与数据库通信.
        ...
        '该SQL语句表示2个一对多,即多对多关系,从连接表中按指定条件(au_id相等的titleauthor记录,title_id相等的记录).     
        '选出指定列(姓,名,书名,价格),并按指定条件(名和姓)升序排序
        objDataAdapter1th.SelectCommand.CommandText = "select distinct 发现过程 from 不良品信息 " &
             "where 不良类型='" & 选择不良类型.Text & "'"
        '这里的SelectCommand的CommandType属性就是CommandType.Text,是默认属性可以省略的.
        objDataAdapter1th.SelectCommand.CommandType = CommandType.Text
        '数据适配器对象开始检索数据并填充到DataSet对象
        'Fill方法的第二参数可以随便填,最好填相关的数据源表,方便理解.  
        ...
        With 选择发现工程
            ...
            Next
        End With
        选择发现工程.SelectedIndex = 0      '选择默认项目为第一项
    End Sub
    ...
    Private Sub 重置条件_Click(sender As Object, e As EventArgs) Handles 重置条件.Click
        ...
        选择不良类型.SelectedIndex = 0  '选择首行项目
        选择发现工程.SelectedIndex = 0  '选择首行项目
    End Sub

    Private Sub 统计分析_Click(sender As Object, e As EventArgs) Handles 统计分析.Click
        'On Error Resume Next
        Dim i As Integer, k As Integer, rngEmpty As Excel.Range
        ...
        '设置工作表标题
        ws.Range("A1").Value = 选择不良类型.Text & " 的不良/成本统计分析"
        With ws.Range("A1:E1")
            .Merge()
            .Font.Size = 15
            .RowHeight = 20
        ...
        End With
        ws.Range("A2:E2").Value = {"发生日期", "不良类型", "发现过程", "不良数量", "损失成本"}
        '从数据表中查询数据
        i = 3
        ...
        For myDateDay As Integer = 0 To (Now.Subtract(起始日期.Value).TotalDays - Now.Subtract(截止日期.Value).TotalDays)
            ...
            ws.Range("B" & i).Value = 选择不良类型.Text
            ws.Range("C" & i).Value = 选择发现工程.Text
            '计算某日指定规格之采购项目的销售总数和销售总额
            If 选择发现工程.Text = "所有过程" Then
                '从加工使用信息表中,按照指定条件(使用日期=变量值,采购项目名称值=控件值),根据所选字段(聚合函数合计值)返回记录集并对字段名称重新命名
                sql = "select sum(不良数量) as aa," &
                    "sum(损失成本) as bb from 不良品信息 " &
                     "where 发生日期=#" & Format(起始日期.Value.AddDays(myDateDay), "yyyy-MM-dd") & "#" &
                   " and 不良类型='" & 选择不良类型.Text & "'"
            Else
                sql = "select sum(不良数量) as aa," &
                     "sum(损失成本) as bb from 不良品信息 " &
                     "where 发生日期=#" & Format(起始日期.Value.AddDays(myDateDay), "yyyy-MM-dd") & "#" &
                     " and 不良类型='" & 选择不良类型.Text & "'" &
                     " and 发现过程='" & 选择发现工程.Text & "'"
            End If
        ...
        For Each rngBuLiang In xlapp.Range("d3:d" & xlapp.Range("a" & xlapp.Rows.Count).End(-4162).Row)

            If Len(rngBuLiang.Value) = 0 Then
                k = k + 1
                If k = 1 Then rngEmpty = rngBuLiang Else rngEmpty = xlapp.Union(rngEmpty, rngBuLiang)
            End If
        ...
        xlapp.Range("A2:E" & xlapp.Range("a" & xlapp.Rows.Count).End(-4162).Row).Select()      '选择区域


        Dim bytFinelRow As Byte = 0

        bytFinelRow = xlapp.ActiveSheet.UsedRange.Rows.Count

        xlapp.Charts.Add()  '新建图表
        xlapp.ActiveChart.ChartType = 51   '活动图表类型为簇状柱形图(本来就是默认的,多设置下保险起见)
        xlapp.ActiveChart.SetSourceData(Source:=
          xlapp.Sheets("Sheet1").Range("A2:A" & bytFinelRow & ",D2:E" & bytFinelRow), PlotBy:=2) '为指定图表设置源数据区域。第一参数表示:包含源数据的区域。第二参数:指定数据绘制方式。可为以下 XlRowCol 常量之一：xlColumns 或 xlRows。
        ...
        With xlapp.ActiveChart
            .HasTitle = True    '显示标题
            .ChartTitle.Characters.Text = 选择不良类型.Text & " 不良成本统计图表"
            .Axes(1, 1).HasTitle = True     '显示纵坐标轴标题   '
            .Axes(1, 1).AxisTitle.Characters.Text = "发生日期"     '重新赋值为日期
            .Axes(2, 1).HasTitle = True   '显示横坐标轴标题
            .Axes(2, 1).AxisTitle.Characters.Text = "不良数量"    '重新赋值为已用数量
            '        .Axes(xlCategory, xlSecondary).HasTitle = False     '次坐标
            .Axes(2, 2).HasTitle = True      '显示次坐标标题
            .Axes(2, 2).AxisTitle.Characters.Text = "不良成本(元)"       '重新赋值为加工使用成本元
            .Axes(2).MajorUnit = 1
            .Axes(1).CategoryType = 2
    ...
    Private Sub 关闭退出_Click(sender As Object, e As EventArgs) Handles 关闭退出.Click
        ...
        Globals.Ribbons.Ribbon1.btn不良信息分析.Enabled = True     '重新使按钮可用.
        Me.Close()  '关闭窗体
    End Sub
    ...
    Private Sub 起始日期_ValueChanged(sender As Object, e As EventArgs) Handles 起始日期.ValueChanged
        不良类型()
    End Sub

    Private Sub 截止日期_ValueChanged(sender As Object, e As EventArgs) Handles 截止日期.ValueChanged
        不良类型()
    End Sub
End Class