...
Public Class C03_采购物品信息管理
    ...
    Dim objDataAdapter As New OleDbDataAdapter("SELECT 采购物品信息.* FROM 采购物品信息 ORDER BY 物品编码", objConnection)
    ...
    Private Sub FillDataSetAndView()
        ...
        objDataAdapter.Fill(objDataSet, "WPXX")  '这里没有设置SelectCommand属性,因为在初始化Adapter对象时,已经使用了相应的参数.
        '设置对应表为数据源绑定到DataView类  Set the DataView object to the DataSet object.
        objDataView = New DataView(objDataSet.Tables("WPXX"))
    ...
    Private Sub C03_采购物品信息管理_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        grdAuthorTitles.DataMember = "WPXX"  'gird控件要显示数据源(填充过数据的DataSet对象)具体的表名称
        ...
        grdAuthorTitles.Columns(4).HeaderText = "最高库存"
        grdAuthorTitles.Columns(5).HeaderText = "最低库存"
        grdAuthorTitles.Columns(6).HeaderText = "备注"
        grdAuthorTitles.Columns(6).Width = 2065 '设置指定列默认宽度小一点
        grdAuthorTitles.Columns(1).Width = 265
        grdAuthorTitles.Columns(2).Width = 265
        ...
        'objDataAdapter1th.SelectCommand.CommandText = "select distinct " & "计量单位" & " from " & "单位信息"
        ...
        'objDataAdapter1th.Fill(objDataSet1th, "wpxx2")
        ''objDataView1th = New DataView(objDataSet.Tables("jhxx2"))
        'Dim tb As DataTable = objDataSet1th.Tables("wpxx2")
        ...
        '    计量单位.Items.Add(tb.Rows(inCounter).Item(0).ToString)   '添加项目值为记录字段所对应的值
        'Next
    ...
    Private Sub 执行排序_Click(sender As Object, e As EventArgs) Handles 执行排序.Click
        ...
        Select Case 排序字段.SelectedIndex
            ...
            Case 4
                objDataView.Sort = "最高库存"
            Case 5
                objDataView.Sort = "最低库存"
            Case 6
                objDataView.Sort = "备注"
        End Select
    ...
    Private Sub 查询条件_TextChanged(sender As Object, e As EventArgs) Handles 查询条件.TextChanged
        If 查询条件.Text.Length = 0 Then              '如果是空值
            C03_采购物品信息管理_Load(Nothing, Nothing)   '调用加载窗体事件.填充数据显示DateGirdVie完整视图,绑定控件,显示当前记录位置
        End If
    End Sub
    ...
    Private Sub 添加_Click(sender As Object, e As EventArgs) Handles 添加.Click
       ...
       ("SELECT TOP 1 * FROM 采购物品信息 ORDER BY 物品编码 DESC", objConnection)
        ...
        Select Case Len(intMaxID.ToString)
            Case 1
                strID = "WP00" & intMaxID.ToString
            Case 2
                strID = "WP0" & intMaxID.ToString
        End Select
        ...
        ' myArray = {"物品编码", "物品名称", "物品规格", "计量单位", "最高库存", "最低库存", "备注"}
        objCommand.CommandText = "INSERT INTO 采购物品信息 " &
        "(物品编码, 物品名称, 物品规格, 计量单位, 最高库存, 最低库存, 备注) " &
        "VALUES(@物品编码, @物品名称, @物品规格, @计量单位, @最高库存, @最低库存, @备注)"
        ...
        objCommand.Parameters.AddWithValue("@物品编码", strID)
        ...
        objCommand.Parameters.AddWithValue("@最高库存", 最高库存.Text)
        objCommand.Parameters.AddWithValue("@最低库存", 最低库存.Text)
        ...
        C03_采购物品信息管理_Load(Nothing, Nothing)
    ...
    Private Sub 更新_Click(sender As Object, e As EventArgs) Handles 更新.Click
        ...
        'myArray = {"物品编码", "物品名称", "物品规格", "计量单位", "最高库存", "最低库存", "备注"}
        objCommand.CommandText = "UPDATE 采购物品信息 " &
            "SET 物品名称 = @物品名称,物品规格 = @物品规格,计量单位 = @计量单位,最高库存 = @最高库存,最低库存 = @最低库存,备注 = @备注 WHERE 物品编码 = @物品编码"
        ...
        objCommand.Parameters.AddWithValue _
            ("@物品编码", BindingContext(objDataView).Current("物品编码"))
    ...
    Private Sub 删除_Click(sender As Object, e As EventArgs) Handles 删除.Click
        ...
        objCommand.Connection = objConnection
        objCommand.CommandText = "DELETE FROM 采购物品信息 " &
            "WHERE 物品编码 = @物品编码"
    ...
    Private Sub 退出_Click(sender As Object, e As EventArgs) Handles 退出.Click
        ...
        Globals.Ribbons.Ribbon1.Button33.Enabled = True     '重新使按钮可用.
        Me.Close()  '关闭窗体
    End Sub

    Private Sub C03_采购物品信息管理_Closed(sender As Object, e As EventArgs) Handles Me.Closed
        '清理内存及数据适配器对象
        objDataAdapter = Nothing           '清理数据适配器对象,释放内存 ' Clean up
        objConnection = Nothing            '清理连接对象,释放内存
        Globals.Ribbons.Ribbon1.Button33.Enabled = True  '重新使按钮可用.
